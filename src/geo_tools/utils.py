...
def import_forest(file_id: str) -> xr.DataArray:
    ...
    Data can be downloaded from https://gitlab.iiasa.ac.at/forestnavigator/wp2/public
...
def clip_array(iso_code: str, xarray_id: str):
    """Clipping xarray data of larger bounds to smaller bounds, specifically this function
    ...
    def to_clip(xarray_w_crs, border_gpd):
        clip_xarray = xarray_w_crs.rio.clip_box(*border_gpd.total_bounds).rio.clip(
            border_gpd.geometry, crs=xarray_w_crs.rio.crs
        )
        return clip_xarray

    if xarray_input.rio.crs is None and border.crs is not None:
        print(
        ...
        # Reprojection the raster file
        ds_reproject = xarray_input.rio.write_crs(shp_border.crs)

        # Clipping the raster
        country_forest = to_clip(ds_reproject, border)

    elif xarray_input is not None and border.crs is None:
        print(
            "CRS information for the boundary shapefile is missing. Reprojecting the boundary shapefile to match the CRS of the raster file"
        )

        # Reprojecting the boundary shapefile
        border = border.set_crs(xarray_input.rio.crs, allow_override=True)

        # Clipping the raster
        country_forest = to_clip(xarray_input, border)

    else:
        print("Either the raster or the boundary shapefile must have a CRS information")

    # Add iso information
    country_forest = country_forest.assign_attrs(
        iso=iso_code, crs=country_forest.rio.crs
    )

    print(f"CRS for the clipped raster is set to {country_forest.rio.crs}")
    print(f"CRS for the boundary shapefile is set to {border.crs.to_epsg()}")
...
def define_class(
    ...
    country_forest, iso_border = clip_array(iso_code, xarray_id)
    ...
    while end <= max_val:
        ...
        calc_area = (
            (forest_data.count(["latitude", "longitude"]).values.flatten()[0]) / 1e6
        )  # in Mha
    ...
    forest_class_df.insert(loc=0, column="Region", value=iso_code)

    # Add a column for var_uid
    var_uid = xarray_id.lower()
    forest_class_df.insert(loc=2, column="Var_uid", value=var_uid)

    # Add a column for year
    forest_class_df.insert(loc=3, column="Year", value=forest_layer_year)

    # Add a column for unit
    forest_class_df.insert(loc=4, column="Unit", value="Million hectares")
